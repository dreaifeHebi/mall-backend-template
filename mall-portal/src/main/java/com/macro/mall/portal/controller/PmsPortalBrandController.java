...
public class PmsPortalBrandController {
    ...
    @ApiOperation("获取品牌信息列表（包含商品数量）")
    @RequestMapping(value = "/brandInfoList", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult<List<PmsPortalBrandInfo>> getBrandInfoList() {
        List<PmsPortalBrandInfo> brandInfoList = portalBrandService.getBrandInfoList();
        return CommonResult.success(brandInfoList);
    }

    @ApiOperation("根据分类ID获取品牌信息列表（包含该分类下的商品数量）")
    @RequestMapping(value = "/brandInfoList/{categoryId}", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult<List<PmsPortalBrandInfo>> getBrandInfoListByCategory(@PathVariable Long categoryId) {
        List<PmsPortalBrandInfo> brandInfoList = portalBrandService.getBrandInfoListByCategory(categoryId);
        return CommonResult.success(brandInfoList);
    }
}
