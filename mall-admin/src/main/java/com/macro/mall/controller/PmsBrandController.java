...
public class PmsBrandController {
    ...
    @ApiOperation("前台更新品牌信息（接收图片URL）")
    @RequestMapping(value = "/frontend/update/{id}", method = RequestMethod.POST)
    @ResponseBody
    public CommonResult<String> updateBrandForFrontend(
        ...
        try {
            int count = brandService.updateBrandForFrontend(id, name, subTitle, logoUrl);
            if (count > 0) {
    ...
    @ApiOperation("前台新增品牌（接收图片URL）")
    @RequestMapping(value = "/frontend/create", method = RequestMethod.POST)
    @ResponseBody
    public CommonResult<Long> createBrandForFrontend(
        ...
        try {
            Long brandId = brandService.createBrandForFrontend(name, subTitle, logoUrl, firstLetter);
